...
pub struct VpuDecoder {
    ...
    // Mmap'd input (compressed) buffers
    input_bufs: Vec<MmapBuffer>,
    input_sizes: Vec<usize>, // sizeimage for each buffer

    // Mmap'd output (raw NV12) buffers
    output_bufs: Vec<Vec<MmapBuffer>>, // [buf_index][plane_index]
    output_sizes: Vec<Vec<usize>>,

    // Dimensions (updated on SOURCE_CHANGE event)
    pub width: u32,
    pub height: u32,

    // Tracks which input buffers are free
...
impl VpuDecoder {
    ...
    fn init_formats(&mut self) -> Result<(), VpuError> {
        // INPUT: H.264 compressed stream
        ...
        unsafe {
            ...
            // Width/height can be 0 for the input side — the VPU reads them
            // from the SPS in the bitstream.
            fmt.fmt.pix_mp.width = 0;
            fmt.fmt.pix_mp.height = 0;
            fmt.fmt.pix_mp.num_planes = 1;
            fmt.fmt.pix_mp.plane_fmt[0].sizeimage = 1 << 20; // 1 MiB per buffer
        }
        ioctl!(self.fd, VIDIOC_S_FMT, "S_FMT(INPUT)", &mut fmt);

        // CAPTURE: NV12 raw output — dimensions will be set by SOURCE_CHANGE
        let mut fmt = V4l2Format {
    ...
    fn alloc_input_buffers(&mut self) -> Result<(), VpuError> {
        ...
        for i in 0..req.count as usize {
            ...
            let size = plane.length as usize;
    ...
    fn alloc_output_buffers(&mut self) -> Result<(), VpuError> {
        let mut req = V4l2RequestBuffers {
    ...
    fn stream_on(&self) -> Result<(), VpuError> {
        let mut t: u32 = V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE;
        ioctl!(self.fd, VIDIOC_STREAMON, "STREAMON(INPUT)", &mut t);
        t = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
        ioctl!(self.fd, VIDIOC_STREAMON, "STREAMON(CAPTURE)", &mut t);

        // Pre-enqueue all output buffers so the VPU can fill them
